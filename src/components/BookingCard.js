...
function BookingCard({image,cardId, product, date, time, price,productId, setbookingUpdate}) {
...
const [formValues, setFormValues] = useState(initialValues);
 const [modalOpenReschedule, setIsOpenReschedule] = useState(false);
    ...
    function deleteFunc(e){
      e.preventDefault();
    firestore.collection("userBookings").doc(`${cardId}`).delete().then(() => {
...
function reschedule(e){
  e.preventDefault();
const serviceRef = firestore.collection("userBookings").doc(`${cardId}`);
